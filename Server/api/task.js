...
module.exports = app => {
    const getTasks = (request, response) => {
        const date = request.query.date ? request.query.date
        ...
        app.db('tasks')
            ...
            .then(rowsDeleted => {
                if (rowsDeleted > 0) {
                    ...
                    const msg = `Não foi encontrada task com id ${request.params.id}.`;
    ...
    const toggleTask = (request, response) => {
        app.db('tasks')
            ...
            .then(task => {
                if (!task) {
                    const msg = `Task com id ${request.params.id} não encontrada.`;
